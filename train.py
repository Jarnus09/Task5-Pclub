...
parser.add_argument('--anneal', type=str, required=True)
...
def train(model, X_train, y_train, X_valid, y_valid, num_epochs, learning_rate, minibatch_size, anneal,loss,momentum,opt):
    ...
    # iterate over minibatches
        ...
        logging.info(f'Epoch: {e+1:03d} 'f'| Loss: {valid_loss:.2f} 'f'| Error: {valid_error:.2f}% ' f'| lr: {learning_rate:.4f}')
        if(anneal == True):
            if(e>0):
...
epoch_loss, epoch_train_acc, epoch_valid_acc, epoch_valid_loss = train(model, X_train1, y_train, X_val1, y_val, 50 ,args.lr,args.minibatch_size,anneal, args.opt,args.momentum,args.opt)
